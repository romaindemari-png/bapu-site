...
function fill(svg,box,U,live){const r=box.getBoundingClientRect();const cols=Math.max(3,Math.round(r.width/U)),baseRows=Math.max(1,Math.round(r.height/U)),crop=(live&&innerWidth<=820)?1:0,rows=Math.max(1,baseRows-crop);
